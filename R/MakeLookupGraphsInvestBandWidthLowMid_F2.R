...
foreach (t = 1:(MaxT-20)) %do% {
  ...
  foreach (s = 0:5) %do% {
    ...
    foreach (f = 0:80) %do% {
      ...
      # invest loop
      OutMid= foreach (i = 0:100, .combine = rbind) %dopar% {FunctMid(t,s,f,i)} 
      #colnames(OutLow) = CN
      colnames(OutMid) = CN
      
      #write.csv(OutLow, "LookupFitLowFunc.csv", row.names = F)
      write.csv(OutMid, "LookupFitMidFunc.csv", row.names = F, append = T)
...
#write.csv(OutLow, "LookupFitLowFunc.csv", row.names = F)
write.csv(OutMid, "LookupFitMidFunc.csv", row.names = F)
