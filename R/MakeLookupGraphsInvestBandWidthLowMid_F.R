...
OutLow2 = matrix(NA,(30*6*81*101),(4+MaxT))
OutMid = matrix(NA,0,(4+MaxT))
OutMid2 = matrix(NA,(30*6*81*101),(4+MaxT))
...
#######################################
MakePropTable = function(t, s, f, i){
  out = matrix(c(rep(0, s), 1, rep(0, 6-s)), 1, 7)
  cn = NULL
  ...
  for(p in t:MaxT){
    ...
    NewRow = c(out[(p-t+1), 1] * (1 - min(1, Adj*Risk[(p-t+1),1])),
               out[(p-t+1), 2] * (1 - min(1, Adj*Risk[(p-t+1),2])) + out[(p-t+1), 1] * min(1, Adj*Risk[(p-t+1),1]),
               out[(p-t+1), 3] * (1 - min(1, Adj*Risk[(p-t+1),3])) + out[(p-t+1), 2] * min(1, Adj*Risk[(p-t+1),2]),
               out[(p-t+1), 4] * (1 - min(1, Adj*Risk[(p-t+1),4])) + out[(p-t+1), 3] * min(1, Adj*Risk[(p-t+1),3]),
               out[(p-t+1), 5] * (1 - min(1, Adj*Risk[(p-t+1),5])) + out[(p-t+1), 4] * min(1, Adj*Risk[(p-t+1),4]),
               out[(p-t+1), 6] * (1 - min(1, Adj*Risk[(p-t+1),6])) + out[(p-t+1), 5] * min(1, Adj*Risk[(p-t+1),5]),
               out[(p-t+1), 7]                                     + out[(p-t+1), 6] * min(1, Adj*Risk[(p-t+1),6]))
    out = rbind.data.frame(out, NewRow)
  } # p loop
  return(out)
}
...
colnames(MidRow) = CN
write.csv(LowRow, "LookupFitLow.csv", row.names = F, append=T)
write.csv(MidRow, "LookupFitMid.csv", row.names = F, append=T)
        OutLow = rbind.data.frame(OutLow, LowRow)
        OutMid = rbind.data.frame(OutMid, MidRow)
        LowOut2[((t-1)*30*6*81*101 + s*6*81*101 + (f-1)*81*101 + i+1),]=c(LowRow)
        MidOut2[((t-1)*30*6*81*101 + s*6*81*101 + (f-1)*81*101 + i+1),]=c(MidRow)
              } # end invest loop
...
write.csv(OutLow2, "LookupFitLow2.csv", row.names = F)
write.csv(OutMid, "LookupFitMidEnd.csv", row.names = F)
write.csv(OutMid2, "LookupFitMId2.csv", row.names = F)
