...
ProbDepFunc::ProbDepFunc(const std::string& func_str,
  ...
  symbol_table.add_variable("prob", prob_);
...
float ProbDepFunc::operator()(unsigned int age, unsigned int shocks,
                              unsigned int fitness, float prob) const {
  age_ = static_cast<double>(age);
  shocks_ = static_cast<double>(shocks);
  fitness_ = static_cast<double>(fitness);
  prob_ = static_cast<double>(prob);
