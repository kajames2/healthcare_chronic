...
UtilityFunc::UtilityFunc(const std::string& func_str,
    ...
    : max_age_(max_age),
      ...
      utility_consts_(utility_consts) {
  expression_str_ = func_str;
  exprtk::symbol_table<double> symbol_table;
  for (const auto& [key, value] : utility_consts_) {
  ...
  symbol_table.add_variable("joy", joy_);
...
double UtilityFunc::operator()(unsigned int age, unsigned int shocks,
                               unsigned int fitness, float joy) const {
  age_ = static_cast<double>(age);
  shocks_ = static_cast<double>(shocks);
  fitness_ = static_cast<double>(fitness);
  joy_ = static_cast<double>(joy);
